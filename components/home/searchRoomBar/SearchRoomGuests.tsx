...
const SearchRoomGuests:React.FC = () => {
    const [popuoOpend, setPopupOpend ] = useState(false);
	...
	//* 어린이 수 변경하기 Dispatch
	const setChildCountDispatch = (value: number) => {
	...
	// 게스트 인원 수 텍스트
	const guestsText = `게스트 ${adultCount + childrenCount}명${
		!infantsCount ? '' : `, 유아 ${infantsCount}명`
	}`;

        return (
        <Container onClick={() => setPopupOpend(true)}>
					...
					<div className='search-room-bar-guests-popup'>
						...
						<div className='search-room-bar-guests-counter-wrapper'>
							<Counter
								...
								description='2~12세'
								value={childrenCount}
                                onChange={(count) => setChildCountDispatch(count)}
							/>
						</div>
						<div className='search-room-bar-guests-counter-wrapper'>
							<Counter
